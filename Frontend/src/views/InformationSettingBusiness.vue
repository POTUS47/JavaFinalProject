                        ...
                        <el-card class="custom-card-width">
                            <el-form :model="userimades" ref="form" class="user-form">
                                ...
                                <el-form-item label="上传简介" prop="description">
                                    <el-input v-model="businessInfo.description" maxlength="20"
...
export default {
    ...
    methods: {
        ...
        async getUserInfo() {
            try {
                const response = await axiosInstance.get('/users/store/myInfo');
                //console.log('获取到的头像和文字描述:', this.userimades);
                this.businessInfo = {
                    username: response.data.data.storeName,
                    store_score: response.data.data.storeScore,
                    hasCertificate: response.data.data.hasCertificate,
        ...
        async updateBusinessInfo() {
            ...
            // 获取需要更新的店铺名和地址
            const newName = this.businessInfo.username ; // 从表单或绑定数据中获取新店铺名
            const newAddress = this.businessInfo.address; // 从表单或绑定数据中获取新地址
            try {
                // 同时发送两个请求
                const [updateNameResponse, updateAddressResponse] = await Promise.all([
                    axiosInstance.put("/users/store/updateStoreName", null, {
                        params: { newName },
                    }),
                    axiosInstance.put("/users/store/updateAddress", null, {
                        params: { newAdress: newAddress },
                    }),
                ]);

                // 根据返回的响应处理结果
                if (
                    updateNameResponse.data.code===200 &&
                    updateAddressResponse.data.code===200
                ) {
                ...
                console.error("更新店铺信息时发生错误：", error);
